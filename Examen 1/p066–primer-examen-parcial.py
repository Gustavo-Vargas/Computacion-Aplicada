...
"""
Objetivo: Un cine local necesita un programa para administrar la venta de boletos par auna función especial en una de sus salas. EL programa debe controlar la venta de boletos, gestionar el aforo y calcylar los ignresos generados. Diseña un porgrama en Python (en base a la plantilla anexa), aplicando los conocimietnos aprendidos en clase, para resolver el problema de acuerdo con los requisitos.
...
[Respuesta aquí]
Se debe de crar una variable donde se almacenara el dia en el que esta registrando las ventasy agregar un if donde valide el dia y agregarlo antes del if, 
despues agregar un fi donde si es martes se haga el descyento y si no es martes no se palique nada, dentro de este if estaria la parte donde se hace el cobro y se agrega el ignreso dependeiedo del tipo de comrpador, (punto 4)
...
[Respuesta aquí]
Priemro iria desd donde se esta guardando los ingresos por cada tipo y probaria solo un tipo de comrpador y veria que si se este generando correctamente en cada tipo
pudiera agregar algun print({ingreso_totales}) para poder ver que esta guardando en cada una de las ventas y ver que si se este sumadno correctamente
